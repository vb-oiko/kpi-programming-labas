...
const b1Text = (y = "y") => `\\ln(1 + ${y})`;
const b1Val = (y) => Math.log(1 + y);
const b1DerText = (y = "y") => `\\frac{1}{1+ ${y}}`;
const b1DerVal = (y) => 1 / (1 + y);

const b2Text = (y = "y") => `2 ${dot(y)} ${y}`;
const b2Val = (y) => 2 * y;
const b2DerText = (y = "y") => `2`;
const b2DerVal = (y) => 2;

const cText = (y = "y") => `\\frac{1}{2} ${dot(y)}  ${y}^2`;
const cVal = (y) => 0.5 * y * y;
const cDerText = (y = "y") => `${y}`;
const cDerVal = (y) => y;
...
const x1PlusX2 = round(cVal(yStar));
...
const lambda1 = round(b2Val(yStar) - cVal(yStar) + b1y2);

const r1Text = "\\frac{1}{y+y^2}";
const r2Text = "\\frac{2}{y}";
const r1Val = (y) => 1 / (y + y * y);
const r2Val = (y) => 2 / y;

const rPoly = "y^2-y-3 = 0";
...
В коаліції $ x_1^* + x_2^* = ${x1PlusX2}$. Отже:
...
$$ x_1^* = \\frac{b_1(y^*)}{2} = ${b1y2}$$

$$ \\lambda_1 = \\frac{1}{n-1} (b_2(y^*)-c+x_1^*) = ${round(
        b2Val(yStar)
      )} - ${round(cVal(yStar))} + ${b1y2} =
${round(b2Val(yStar) - cVal(yStar) + b1y2)} $$
...
$$ x_2^* = \\frac{b_2 (y^*)}{2} -\\lambda^* = ${b2y2 * 2} - ${lambda1} =
${round(b2y2 * 2 - lambda1)} $$
...
$$ x_1^* + x_2^* = ${round(cVal(yStar) * r1 + cVal(yStar) * r2)} $$
...
${round(b1Val(yStar) - b1Val(yOver) + b2Val(yStar) - b2Val(yOver))}
 $$
