...
- забезпечення обов'язковий контроль доступу (MAC) за допомогою утиліт AppArmor SELinux, які доповнює традиційну модель дискреційного контролю доступу (DAC) Unix.
- видалення вразливих (FTP, telnet, та rlogin) та непотрібних програмних пакетів.
- налаштування автоматичного оновлення через Інтернет.
- обмеження доступу до знімних носіїв. У деяких середовищах потрібно обмежити доступ до знімних носіїв, таких як USB-накопичувачі або оптичні пристрої.
- увімкнення старіння паролів
- використання набору спільних бібліотек Linux-PAM (Pluggable Authentication Modules for Linux), які дозволяють локальному системному адміністратору вибирати, як програми автентифікують користувачів.
- обмеження входу в систему як кореневого користувача.
- встановлення тайм-ауту неактивності для інтерактивних сеансів оболонки.
- запобігання випадкової відмови в обслуговуванні - встановлення обмеження на кількість системних ресурсів, які можуть споживати користувачі та групи.
- розміщення банер на екранах входу на всіх серверах з міркувань юридичної/аудиторської політики та для відлякування зловмисників тощо.
